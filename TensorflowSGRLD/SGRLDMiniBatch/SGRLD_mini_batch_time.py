...
from util import set_seed, printing
...
def main(_):
    ...
    parser.add_argument('-t', '--time_sec', dest="Time_sec", type=float,
                        help="Time after which auc and perplexity is computed")
    parser.add_argument('-mt', '--max_time_sec', dest="MaxTime_sec", type=float,
                        help="Maximum Time to run the MCMC")
    ...
    MaxTime_sec = args.MaxTime_sec
    seed = args.seed

    outfile = outfile + network + 'Seed' + str(seed) + 'L' + str(L) + 'M' + str(M) + 'K' + str(k) + 'test_ratio' + str(
        Test_ratio) + 'Time.npz'
    ...
    e_tf = tf.constant(2*Num_edges, dtype=tf.float32, name="e_tf")
    ...
    with tf.Session() as sess:
        ...
        while((stop-start) < MaxTime_sec):
            ...
            # SAMPLING ALPHA
            sess.run(find_alpha)
            # COMPUTING AUC and perplexity
            stop_iter = timeit.default_timer()
            if (stop_iter-start_iter) > Time_sec:
                start_iter = start_iter + 30  # Reset start time
                # compute auc and perplexity
    ...
    print(len(TestAUCvector))
    np.savez(outfile, TestAUCvector=TestAUCvector,TestPPvector=TestPPvector,TrainAUCvector=TrainAUCvector,TrainPPvector=TrainPPvector)
